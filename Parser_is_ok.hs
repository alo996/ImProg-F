...
--Parser errors should be dropped when we expect something and it is not there
  ...
  --reached only via expr from def, needs implementation of LocalDefinition
  localDef :: Parser Expression
...
--reached only via expr from def, needs implementation of LocalDefinitions
  localDefs :: Parser [Expression]
  localDefs xs = do
