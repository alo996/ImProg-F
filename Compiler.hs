...
compileLocalDefinitions ldefs e pos = compileLocalDefinitions' ldefs e (replicate' [Alloc, Alloc, Makeapp] len) pos' len len
  where
    ...
    compileLocalDefinitions' ((LocalDef e1 e2) : defs) e ccells pos' n acc =
        compileLocalDefinitions' defs e (ccells ++ compileExpression e2 pos' ++ [UpdateLet $ acc-1]) pos' n (acc-1)
