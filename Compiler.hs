module Compiler where
    ...
    compileDefinition (Def e1 es e2) s@State{code = Code ccells, global = GlobalEnv hcells} = 
        -- First we create a local environment for the function definition. Is this the right way to go?
        let localenv = trace ("compileDefinition calling posList with es = " ++ show es) posList es in
        ...
        s {code = Code (ccells ++ trace ("compileDefinition calling compileExpression with e2 = " ++ show e2 ++ " and localenv = " ++ show localenv) compileExpression e2 0 localenv ++ [Slide (length localenv + 1), Reduce, Return]), global = push (global s) (DEF (show e1) (length localenv) (depth (code s)))}
